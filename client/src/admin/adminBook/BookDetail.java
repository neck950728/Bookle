...
public class BookDetail extends JDialog {
	...
	private String[] headersRental = new String[] { "대여 일자", "회원 ID", "이름", "전화번호", "이메일" };
	...
	private String[] headersReservation = new String[] { "예약 일자", "회원 ID", "이름", "전화번호", "이메일" };
	...
	private String[] headersStandby = new String[] { "대기 번호", "회원 ID", "이름", "전화번호", "이메일", };
	...
	public void listUpdate() throws Exception {
		...
		for (int i = 0; i < listSize; i++) {
			...
			String member_phone = dis.readUTF();
			String member_email = dis.readUTF();
			if (return_date.equals("대여 중")) {
				dtmRental.addRow(new Object[] { rental_date, member_id, member_name, member_phone, member_email });
			}
		}
		...
		for (int i = 0; i < listSize; i++) {
			...
			String member_phone = dis.readUTF();
			String member_email = dis.readUTF();
			dtmReservation.addRow(new Object[] { reserved_date, member_id, member_name, member_phone, member_email });
		}

		dos.writeUTF("list_standby_b");
		dos.writeUTF(id);
		dos.flush();
		...
		for (int i = 0; i < listSize; i++) {
			...
			dtmStandby.addRow(new Object[] { order_number, member_id, member_name, member_phone, member_email });
		}
	}
	...
	private void compInit() {
		...
		this.labelTitle.setText(title + "(" + id + ")");
	...
	public BookDetail(JPanel parent, Socket sock, String id, String title, String quantity) {
		this.sock = sock;
		this.id = id;
		this.title = title;
		this.quantity = quantity;
		this.setTitle("도서별 대여/예약 정보");
