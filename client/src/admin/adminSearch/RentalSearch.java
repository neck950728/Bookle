...
public class RentalSearch extends JDialog {

	private JDialog parent = null;
	private RentalSearch self = this;
	...
	public void listUpdate() throws Exception {
		...
		for (int i = 0; i < listSize; i++) {
			...
			dtm.addRow(new Object[] { rental_date, book_title + " (" + book_id + ")",
	...
	private void eventInit() {
		...
		this.buttonSelect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					String rental_date = (String) dtm.getValueAt(table.getSelectedRow(), 0);
					...
					if (parent instanceof ReturnInsert) {
						((ReturnInsert) parent).setFieldRentalDate(rental_date);
						((ReturnInsert) parent).setFieldReturnBook(book_id);
						((ReturnInsert) parent).setFieldReturnMember(member_id);
					}
					dispose();
				} catch (Exception ex) {
	...
	public RentalSearch(JDialog parent, Socket sock) {
		this.parent = parent;
		this.sock = sock;
		this.setTitle("대여 정보 검색");
